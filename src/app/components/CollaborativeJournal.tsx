    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <BookHeart className="w-6 h-6 text-primary-500" />
        ...
        <div className="space-y-8">
          {Object.entries(groupedEntries).map(([date, dateEntries]) => (
            <div key={date} className="relative">
              {/* Date Header */}
              <div className="sticky top-16 z-10 bg-gradient-to-r from-primary-100 to-primary-200 rounded-lg px-4 py-2 mb-4 border border-primary-200">
                <h3 className="font-semibold text-primary-900">{formatTimelineDate(dateEntries[0].createdAt)}</h3>
              </div>

              {/* Timeline Line */}
              <div className="absolute left-8 top-16 bottom-0 w-0.5 bg-gradient-to-b from-primary-300 to-primary-300" />
                ...
                {entry.mediaFiles && entry.mediaFiles.length > 0 && (
                  <div className="mt-3 flex gap-2">
                    {entry.mediaFiles.slice(0, 3).map((media: any, idx: number) => (
                      <div key={idx} className="w-12 h-12 rounded bg-primary-100 flex items-center justify-center">
                        {media.type === 'image' && <ImageIcon className="w-5 h-5 text-primary-600" />}
                        {media.type === 'video' && <Video className="w-5 h-5 text-primary-600" />}
                        {media.type === 'audio' && <Mic className="w-5 h-5 text-primary-600" />}
                      </div>
                    ))}
