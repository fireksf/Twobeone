      ...
      <Card>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              {users.map((user) => {
                ...
                return (
                  <div 
                    key={user.id} 
                    className={`p-3 rounded-lg text-sm ${needsFix ? 'bg-red-50 border-2 border-red-300' : 'bg-gray-50'}`}
                  >
                    <div><strong>ID:</strong> {user.id}</div>
                    <div><strong>Name:</strong> {user.name}</div>
                    <div><strong>Email:</strong> {user.email}</div>
                    <div><strong>Partner ID:</strong> {user.partnerId || 'None'}</div>
                    <div className={needsFix ? 'text-red-600 font-bold' : ''}>
          ...
          <div>
            ...
            <div className="space-y-2">
              {couples.map((couple) => (
                <div key={couple.id} className="bg-blue-50 p-3 rounded-lg text-sm">
