...
export function BottomCTA({ 
  ...
  return (
    <div className={`
      ...
      dark:from-neutral-950 dark:via-neutral-950
      pt-6
      ${className}
    `}>
...
export function TouchableButton({ 
  ...
  const variantStyles = {
    ...
    secondary: 'bg-muted hover:bg-neutral-200  dark:hover:bg-neutral-700 text-foreground dark:text-white',
...
export function FormField({ 
  ...
  return (
    <div className="space-y-2">
      ...
      {children}
      {error && (
        <p className="text-sm text-primary-600 dark:text-primary-400">{error}</p>
      )}
      {helperText && !error && (
        <p className="text-sm text-muted-foreground dark:text-muted-foreground">{helperText}</p>
      )}
    </div>
  );
}
