...
export function MoodAnalytics({ profile, partner, onClose }: MoodAnalyticsProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.warn('Mood AI analysis unavailable:', error?.message);
      ...
      const userAvg = userMoods.length > 0
        ? (userMoods.reduce((s, m) => s + (moodValues[m.mood] || 0), 0) / userMoods.length).toFixed(1)
        : '—';
      const partnerAvg = partnerMoods.length > 0
        ? (partnerMoods.reduce((s, m) => s + (moodValues[m.mood] || 0), 0) / partnerMoods.length).toFixed(1)
        : '—';
  ...
  const MOOD_CONFIG = {
    great: { emoji: '🤩', label: t.mood.great, desc: t.mood.great, bg: 'var(--success-50)',    border: 'var(--success-500)',    color: 'var(--success-700)' },
    good:  { emoji: '😊', label: t.mood.good,  desc: t.mood.good,  bg: 'var(--secondary-50)',  border: 'var(--secondary-500)',  color: 'var(--secondary-700)' },
    okay:  { emoji: '😐', label: t.mood.okay,  desc: t.mood.okay,  bg: 'var(--warning-50)',    border: 'var(--warning-500)',    color: 'var(--warning-700)' },
    sad:   { emoji: '😔', label: t.mood.sad,   desc: t.mood.sad,   bg: 'var(--neutral-100)',   border: 'var(--neutral-400)',    color: 'var(--neutral-600)' },
  } as const;
    ...
    <div className="space-y-6">
          ...
          <AlertDescription className="ml-2">
            <div className="space-y-2">
              ...
              <ul className="text-sm text-warning-700 list-disc list-inside space-y-1 ml-2">
                <li>Add credits to your OpenAI account at <a href="https://platform.openai.com/account/billing" target="_blank" rel="noopener noreferrer" className="underline font-medium hover:text-warning-700">platform.openai.com/account/billing</a></li>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold">{t.mood.analytics}</h1>
            <p className="text-sm text-muted-foreground">{t.mood.analyticsDescription}</p>
          </div>
        </div>
        {partner && (
          <Button
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardDescription>Your Average (30 days)</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center gap-3">
      ...
      {partner && last7DaysData.some(d => d.you !== null || d.partner !== null) && (
        <Card>
          ...
          <CardContent style={{ paddingTop: 'var(--spacing-3)', paddingBottom: 'var(--spacing-3)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-3)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--spacing-2)' }}>
                {[
                  { label: 'Great',    bg: 'var(--success-500)' },
                  { label: 'Good',     bg: 'var(--secondary-500)' },
                  { label: 'Okay',     bg: 'var(--warning-500)' },
                  { label: 'Sad',      bg: 'var(--error-500)' },
                  { label: 'No entry', bg: 'var(--neutral-200)' },
                ].map(({ label, bg }) => (
                  <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <div style={{ width: 8, height: 8, borderRadius: 2, background: bg, flexShrink: 0 }} />
                    <span style={{ fontSize: 'var(--text-label)', color: 'var(--muted-foreground)' }}>{label}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--spacing-4)' }}>
                {[
                  { name: profile?.name || 'You',     key: 'you'     as const },
                  { name: partner?.name || 'Partner', key: 'partner' as const },
                ].map(({ name, key }) => {
                  const moodColor = (val: number | null) => {
                    if (val === null || val === undefined) return 'var(--neutral-200)';
                    if (val >= 3.5) return 'var(--success-500)';
                    if (val >= 2.5) return 'var(--secondary-500)';
                    if (val >= 1.5) return 'var(--warning-500)';
                  ...
                  return (
                    <div key={key}>
                      <p style={{ fontSize: 'var(--text-caption)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--foreground)', marginBottom: 'var(--spacing-1)' }}>
                        {name}
                      </p>
            ...
            {analysis ? (
              ...
              >
                <div className="flex items-start gap-2 mb-3">
                  {analysis.isFallback
                    ? <Brain className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: 'var(--muted-foreground)' }} />
                    : <Sparkles className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: 'var(--warning-500)' }} />
                  }
                  <div className="flex-1">
                    <p className="text-sm font-medium mb-2" style={{ fontWeight: 'var(--font-weight-semibold)', color: 'var(--foreground)' }}>
                      {analysis.isFallback ? 'Mood Summary' : 'AI Insights'}
                    </p>
                    <div className="whitespace-pre-wrap leading-relaxed" style={{ fontSize: 'var(--text-callout)', color: 'var(--foreground)' }}>
                      {analysis.analysis}
                    </div>
                  </div>
                </div>
                <Separator className="my-3" />
                <div style={{ fontSize: 'var(--text-label)', color: 'var(--muted-foreground)', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <Clock className="w-3 h-3" />
                  {analysis.createdAt
                    ? (() => {
                        const d = new Date(analysis.createdAt);
                        return isNaN(d.getTime()) ? 'Just now' : d.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
                      })()
                    : 'Just now'}
                  {analysis.isFallback && (
                    <span style={{ marginLeft: 8, color: 'var(--muted-foreground)' }}>· Stats-based</span>
                  )}
                </div>
              </div>
            ) : (
              <div className="text-center py-6">
                <Brain className="w-12 h-12 mx-auto mb-3" style={{ color: 'var(--primary-300)' }} />
                <p className="text-sm mb-4" style={{ color: 'var(--muted-foreground)', fontSize: 'var(--text-callout)' }}>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1">
                          ...
                          {mood.note && (
                            <p className="text-sm text-foreground mt-2">{mood.note}</p>
                          )}
                        </div>
                      </div>
